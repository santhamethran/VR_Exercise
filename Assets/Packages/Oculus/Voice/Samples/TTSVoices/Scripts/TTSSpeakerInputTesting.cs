...
namespace Meta.Voice.Samples.TTSVoices
{
    public class TTSSpeakerInputTesting : MonoBehaviour
    {
        ...
        [SerializeField] public string[] _queuedText;
        ...
        // Speak phrase click
        public void SpeakClick(int subtitleIndex)
        {
            // Speak phrase
           /* string phrase = FormatText(_input.text);
            ...
            if (_queuedText != null && _queuedText.Length > 0)
            {
                /* foreach (var text in _queuedText)
                 {
                     _speaker.SpeakQueued(FormatText(text));
                 }*/
                _speaker.SpeakQueued(_queuedText[subtitleIndex]);
            }
        }
        // Speak async
